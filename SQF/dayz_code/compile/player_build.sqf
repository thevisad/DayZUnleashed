...
antiBuildables 	= ["Hhedgehog_concrete"];//Object array to prevent building near these objects (great to defend against players who use objects to get inside of base and then remove when leave)
...
_playerCombat = player;
	...
	*/
	...
	//Check if antiBuildables around
	_antiBuildables = (nearestObjects [_location, ["antiBuildables"], 10]);
	_antiBuildable = _antiBuildables select 0; 
	if (count _antiBuildables > 0) then 
	{
		cutText [format["You cannot build %1 within %2 meters of %3",_text, 10, typeOf(_antiBuildable)], "PLAIN DOWN",1];call _funcExitScriptCombat;
	};
