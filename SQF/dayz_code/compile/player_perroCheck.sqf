...
_listTalk = (position _refObj) nearEntities [["Pastor","Fin"], _minDist]; //originally 200, need to tweak aggro distance to generate up to max 200 minium aggro
//diag_log("PPC: _listTalk: " + str(_listTalk));
...

{
	...
	if (alive _x) then {
		...
		//diag_log("PPC: Agent " + str(_x) + " Ref Object: " + str(_refObj));
		if ((_x distance player < dayz_areaAffect) and !(animationState _x == "ZombieFeed")) then {
			//diag_log("PPC: within check");
			//perform an attack
			_last = _x getVariable["lastAttack", 0];
			if ((diag_tickTime - _last) > 2) then {
				//diag_log("PPC: within _last");
				_cantSee = [ _refObj,_x] call dayz_losCheck;
				//diag_log("PPC: Agent " + str(_x) + " Base Cant See: " + str(_cantSee));
				if (!_cantSee) then {
					//diag_log("PPC: within !_cantSee");
					_attackResult = [_x,  _type] call player_perroAttack;
				};
				//diag_log(format["PPC: Agent: %1 %2 %3 / as:%4 up:%5 ur:%6 sp:%7",  _agent,  _x,  _attackResult,  animationState player,  unitPos player,  unitReady _x,  [0, 0, 0] distance (velocity player)]);
			...
			//diag_log("PPC: outside check");
			if (speed _x < 4) then {
				[_x,"dog_bark",(_chance + 4),true] call dayz_zombieSpeak;
			} else {
				[_x,"dog_growl",(_chance + 3),true] call dayz_zombieSpeak;
			};
		};
			...
			if (_dist < DAYZ_disAudial) then {
				if (DAYZ_disAudial > 80) then { //65 Orignal: 80
					...
					_chance = [_x, _dist, DAYZ_disAudial] call dayz_losChance;
						//diag_log ("PPC: " + str(_x) + " Noise Detection/distance: " + str([_refObj, _dist]) + " " + str(_chance));
					if ((random 1) < _chance) then {
						_cantSee = [ _refObj,_x] call dayz_losCheck;
							//diag_log("PPC: Agent " + str(_x) + " Noise Cant See: " + str(_cantSee));
						if (!_cantSee) then {
							_targets set [count _targets,  driver _refObj];
							_group setVariable ["targets", _targets, true];
						} else {
			...
			if (!(_refObj in _targets)) then {
				if (_dist < (DAYZ_disVisual / 2)) then {
					_chance = [_x, _dist, DAYZ_disVisual] call dayz_losChance;
						//diag_log ("PPC: " + str(_x) + " Visual Detection/distance: " + str([_refObj, _dist]) + " chance: " + str(_chance));
					if ((random 1) < _chance) then {
						...
						if (_inAngle) then {
							//LOS check
							_cantSee = [ _refObj, _x] call dayz_losCheck;
								//diag_log("PPC: Agent " + str(_x) + " Sight Cant See: " + str(_cantSee));
							if (!_cantSee) then {
	...
	//diag_log("PPC: Agent: " + str(_x) + " targets list: " + str(_targets));
} forEach _listTalk;
