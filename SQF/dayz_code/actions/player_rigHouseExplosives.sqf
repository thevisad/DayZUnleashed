...
if( _argument == 0 ) then { player removeAction churchie_rig_veh; churchie_rig_veh = -1; player removeMagazine "PipeBomb"; 
	_dir = getDir player; 
	_playerPos = getPos player; 
	_position = [(_playerPos select 0) + (0.90 * sin(_dir)), (_playerPos select 1) + (0.90 * cos(_dir)), _playerPos select 2]; 
	_pipe = "PipeBomb" createVehicle _position; _position = [(_playerPos select 0) + (0.70 * sin(_dir)), (_playerPos select 1) + (0.70 * cos(_dir)), _playerPos select 2]; 
	_bomb = "BAF_ied_v1" createVehicle _position; _pipe setDir (_dir + 250); _bomb setDir (_dir + 200); 
	player playActionNow "Medic"; [player,"repair",0,false] call dayz_zombieSpeak; sleep 6; 
	_nearBomb = (_playerPos) nearObjects["BAF_ied_v1", 10]; 
	_bomb2 = _nearBomb select 0; 
	churchieseventrig = _vehicle addEventHandler ["Engine", {null = [_this select 0, 0, 0, [_vehicle, 2, _bomb]] execVM "\z\addons\dayz_code\actions\player_rigVehicleExplosives.sqf"} ]; cutText ["You have rigged the vehicles engine to blow on ignition.", "PLAIN DOWN"]; 
}; 
