    ...
    while {true} do
    {
        if (!isNull player) then {
            private ["_currentVehicle", "_isNearFeed", "_countFuel"];
                    ...
                    _countFuel = (count ((position _currentVehicle) nearObjects ["Land_A_FuelStation_Shed", _distance]));
					...
					// FuelStation = Land_A_FuelStation_Shed
					//Old ! land_nav_pier_c_big
            ...
            if (_vehicle != _currentVehicle) then {
                ...
                if (_currentVehicle != player && _isNearFeed && !(_currentVehicle isKindof "Bicycle")) then {  //change "Bicycle" to "Land" to allow only air vehicles to aut-refuel
                    ...
                    _vehicle_refuel_id = _vehicle addAction ["Refuel and Repair", "\z\addons\dayz_code\actions\vehiclerepair.sqf", [_amount,_amount2], -1, false, true, "", "vehicle _this == _target && local _target"];
                };
            };
