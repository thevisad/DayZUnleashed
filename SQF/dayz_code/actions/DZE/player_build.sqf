...
if((count ((position player) nearObjects ["All",30])) >= DZE_BuildingLimit) exitWith {CodeInprogress = false; cutText ["\n\nCannot build, too many objects witin 30m.", "PLAIN DOWN"];};
...
if (_inVehicle) exitWith {CodeInprogress = false; cutText ["Cannot build while in a vehicle.", "PLAIN DOWN"];};
...
// _message
if(!_canBuildOnPlot) exitWith {  CodeInprogress = false; cutText [format["Unable to build %1 nearby.",_needText,_distance] , "PLAIN DOWN"]; };
