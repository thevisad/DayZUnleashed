...
moptions = 
{
	...
	[
		...
		[format["Show nearby weapon holders: %1",wpa], [6], "", -5, [["expression", "wpa = !wpa;"]], "1", "1"],
...
drawic = 
{
	if (!isnil "mapm") then 
	{
		...
		if wpa then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\markers\dot.paa", zmc, getPosASL _x, _iscale*30, _iscale*30, getDir _x, "", 1];};} foreach weaponholders;};
...
{	
 	...
 	if (!isnil "mapm") then 
	{
		...
		if (wph) then {weaponholders = getPosATL player nearObjects ["ReammoBox", _zdist];} else {weaponholders = [];};
